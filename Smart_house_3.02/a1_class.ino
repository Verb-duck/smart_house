  ...
  void work(float temperature) {
    ...
    if(request_sensor ()) {
       if ( !flag_retenion_temp) {             //и подгоняем температуру
         if ( temperature_set > temperature_now)  flag_heater = true;
         else   flag_heater = false;
         if ( temperature_set - 1 == temperature_now) {
          flag_retenion_temp = true; //когда встала нужная температура
          timer_ratio = 0;
         }
       }                                            //включаем режим удержания    
       else {                                //режим удержания заданной температуры
        if (temperature_now > temperature_set && timer_ratio > -5000) timer_ratio -= 1000;                  //корректировка коэффициента
        else if (temperature_now < temperature_set && timer_ratio < 5000) timer_ratio += 1000 ; 
